...
static inline drive_status_t drive_get_status() {
  drive_status_t status = { .raw=inb(DRIVE_STATUS_PORT) };
  return status;
}

static inline drive_error_t drive_get_error() {
  drive_error_t error = { .raw=inb(DRIVE_ERROR_PORT) };
  return error;
}

static inline drive_info_t drive_get_info() {
  drive_info_t info = { .raw=inb(DRIVE_REGISTER_PORT) };
  return info;
}
...
static int drive_read_sector_to(ptr_t location, uint32_t lba28) {
  while(drive_get_status().busy)
    io_wait();
  if(lba28 > __drive_current_partition.size)
    return 0;
  lba28 += __drive_current_partition.lba_first;
  outb(DRIVE_REGISTER_PORT, 0xE0 | ((lba28 >> 24) & 0xF)); // 28b LBA, master 
  outb(DRIVE_SECTOR_COUNT_PORT, 1);
  ...
  insw(0x1f0, location, 256);
  //__asm__ __volatile__ ("cld; rep insw" : : "D"((uint32_t)location), "c"(256), "d"(0x1f0));
  return 512;
}

static int drive_read_sectors_to(ptr_t location, uint32_t lba28, uint8_t number_of_sectors) {
  int sectors_read = 0;
  for(uint8_t i = 0; i < number_of_sectors; i++) {
    int tmp = drive_read_sector_to(location+i*512, lba28+i);
    if(tmp == -1) return -1;
    if(!tmp) break;
    sectors_read += tmp;
  }
...
static int drive_read_to(ptr_t location, uint32_t lba28, size_t size) {
  static uint8_t buffer[DRIVE_SECTOR_SIZE];
  uint16_t sectors;
  uint8_t over;
  int total = 0;
  for(over = 0; over < (size / DRIVE_SECTOR_SIZE) / UINT8_MAX; over++) {
    int tmp = drive_read_sectors_to(location+over*DRIVE_SECTOR_SIZE*UINT8_MAX, lba28+over*UINT8_MAX, 0);
    if(tmp == -1) return -1;
    total += tmp;
  }
  ...
  if(sectors) {
    int tmp = drive_read_sectors_to(location+over*DRIVE_SECTOR_SIZE*UINT8_MAX, lba28+over*UINT8_MAX, sectors);
    if(tmp == -1) return -1;
    total += tmp;
  }

  int tmp = drive_read_sectors_to(buffer, lba28+over*UINT8_MAX+sectors, 1);
  if(tmp == -1) return -1;
  total += tmp;
